...
def process_ticker(code, name, start_str, end_str, is_good_market):
    ...
    for attempt in range(max_retries):
        try:
            ...
            is_above_ma200 = latest['Close'] > latest['MA200']
            is_breakout = latest['Close'] > latest['High_20'] if pd.notna(latest['High_20']) else False
            
            signals = []
            if vol_ratio >= 2.0: signals.append(f"🔥 出来高 ({round(vol_ratio, 1)}倍)")
            ...
            history_data = []
            for date_index, row in df.tail(120).iterrows():
                history_data.append({
            ...
            group = "A" if (is_good_market and is_yosen and is_above_ma200 and is_breakout) or ("底打ち確認" in str(signals)) else "B"
            ...
            return {"group": group, "data": {
                "code": code, "name": name, "price": int(latest['Close']), "price_diff": price_diff, "signals": signals, "history_data": history_data,
                "rsi": rsi, "vol_text": f"{latest['Volume']/10000:.1f}万株", "ai_comment": ai_comment, "position": "200日線上" if is_above_ma200 else "200日線下"
            }}
        except:
...
def scan_b_type(target_date_str=None):
    # watchlist.jsonを動的に読み込むように修正
    try:
        with open("watchlist.json", "r", encoding="utf-8") as f:
            watchlist = json.load(f)
    except:
        watchlist = SCAN_UNIVERSE
